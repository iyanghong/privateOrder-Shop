...
<#list allowSearchColumns as column>
    ...
    public Result<Page<${upperCamelCaseName}>> selectPage(@PathVariable("pageNumber") Integer pageNumber, @PathVariable("pageSize") Integer pageSize<#list allowSearchColumns as column>,${column.javaType} ${column.lowerCamelCaseName}</#list>) {
...
<#if primaryKeyColumn??>
    ...
    */
    ...
    public Result<String> delete(@PathVariable("${primaryKeyColumn.lowerCamelCaseName}") ${primaryKeyColumn.javaType} ${primaryKeyColumn.lowerCamelCaseName}) {
        OnlineUser onlineUser = SpringUtil.getOnlineUser();
